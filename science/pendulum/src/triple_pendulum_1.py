...
class TriplePendulum:
    def __init__(self, L1=1.0, L2=1.0, L3=1.0, m1=1.0, m2=1.0, m3=1.0):
        ...
        self.state = np.array([np.pi/2, np.pi/2, np.pi/2, 0.0, 0.0, 0.0])
        
    def derivatives(self, state, t):
        """Calcula as derivadas do sistema"""
        th1, th2, th3, w1, w2, w3 = state
        
        # Simplificação das equações de movimento (usando aproximação numérica)
        # Para um pêndulo triplo completo, as equações são muito complexas
        ...
        num1 = -M * self.g * np.sin(th1) - self.m2 * self.L2 * w2**2 * s12 - self.m3 * self.L3 * w3**2 * s13
        num2 = -(self.m2 + self.m3) * self.g * np.sin(th2) + self.m2 * self.L1 * w1**2 * s12 - self.m3 * self.L3 * w3**2 * s23
        num3 = -self.m3 * self.g * np.sin(th3) + self.m3 * self.L2 * w2**2 * s23 + self.m3 * self.L1 * w1**2 * s13
        ...
        return np.array([w1, w2, w3, alpha1, alpha2, alpha3])
    
    def step(self, dt):
        """Integração usando método de Runge-Kutta de 4ª ordem"""
        k1 = self.derivatives(self.state, 0) * dt
        k2 = self.derivatives(self.state + k1/2, 0) * dt
        k3 = self.derivatives(self.state + k2/2, 0) * dt
        k4 = self.derivatives(self.state + k3, 0) * dt
    ...
    def get_positions(self):
        ...
        x3 = x2 + self.L3 * np.sin(th3)
        y3 = y2 - self.L3 * np.cos(th3)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # Energia (aproximada)
            E = pendulum.m1 * pendulum.g * (-pendulum.L1 * np.cos(th1))
            E += pendulum.m2 * pendulum.g * (-pendulum.L1 * np.cos(th1) - pendulum.L2 * np.cos(th2))
            E += pendulum.m3 * pendulum.g * (-pendulum.L1 * np.cos(th1) - pendulum.L2 * np.cos(th2) - pendulum.L3 * np.cos(th3))
